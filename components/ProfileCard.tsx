...
const ProfileCard = async ({ session }: { session: Session }) => {
  ...
  return (
    <div className="relative max-h-[60rem] gradient-vertical mx-auto flex max-w-xl flex-col gap-12 pt-32 rounded-3xl px-10 py-10 text-white max-xs:px-3">
      ...
      <div className="flex gap-4 items-center">
        <div className="h-20 w-20 max-xs:w-16 max-xs:h-16 bg-[#333c5c] bg-opacity-20 rounded-full items-center justify-center flex">
          <Avatar className="h-16 w-16 max-xs:w-12 max-xs:h-12">
        ...
        <div className="flex flex-col gap-2">
          {student.status === "APPROVED" ? (
            <div className="flex items-center gap-1">
              <Image
                ...
                height={14}
              />
              <p className="text-light-100 text-lg max-xs:text-sm">
                {" "}
                Verified Student
              </p>
            </div>
          ...
          <div>
            <p className="font-bold text-2xl max-xs:text-lg">
              {getFirstName(student.fullName)}
            </p>
            <p className="text-light-100 max-xs:text-sm">{student.email}</p>
          </div>
        </div>
      </div>
      <div className="flex flex-col gap-4">
        <p className="text-light-100 text-lg max-xs:text-sm">University</p>
        <p className="font-bold text-2xl max-xs:text-lg">
          Koforidua Technical University
        </p>
      </div>
      <div className="flex flex-col gap-4">
        <p className="text-light-100 text-lg max-xs:text-sm">Student ID</p>
        <p className="font-bold text-2xl max-xs:text-lg">
